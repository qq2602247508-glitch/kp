...
页、全局案件选择、案件引用、AI 引用、审计日志和交付操作入口。
...
2026-07-26 另通过 Codex 内置浏览器对真实运行中的 `5180` 页面做了只读验收：
确认仪表板读取 20 个来源包和 5,484 个索引片段、顶栏连接真实 `8010` 服务、
审计日志入口可访问、设置页五项就绪状态来自后端。该验收没有向真实数据库写入
演示案件，也不替代未来应加入仓库的可重复浏览器 E2E。
